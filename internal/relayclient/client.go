...
func resultMessages(requestID string, result []byte, maximumMessageBytes, maximumResultBytes int) ([]relay.Envelope, error) {
	...
	response, err := relay.NewEnvelope(relay.MessageTypeResponse, requestID+"-response", relay.ResponsePayload{
	...
	for offset, sequence := 0, uint64(0); offset < len(result); sequence++ {
		...
		for {
			...
			envelope, err = relay.NewEnvelope(relay.MessageTypeStreamChunk, requestID+"-chunk", relay.StreamChunkPayload{
