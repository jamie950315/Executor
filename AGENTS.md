...
- Unified Dashboard device enrollment and the device-side relay foundation are implemented locally: config v2 migration, a persistent P-256 device identity, one-time token-file enrollment, signed WebSocket authentication and generation refresh, locally revalidated device grants, bounded and cancellable existing-MCP dispatch, encrypted Rotate/Kill recovery delivery, generation revocation, and a Dashboard-owned relay that preserves and refreshes the loopback rescue surface. The production web UI, clone deployment automation, live multi-host rollout, and release remain later tasks.
