...
These changes were developed on `pro-fix` and are now included in `main`.
See `2026-09-profix-upgrade.md` for the installed release and live verification.
The repository merge does not change installed services, credentials or Tunnels.
Authenticated owner/admin capabilities retain their existing scope.
...
The complete Dashboard suite now passes 247 tests with one platform-specific
skip, alongside type checking, lint and the build. Both `executor` and
`executor-kill` compile for darwin/amd64, darwin/arm64, linux/amd64,
linux/arm64, windows/amd64 and windows/arm64. The main merge also passed the
full Go suite, focused race tests, vet, build and isolated stdio runtime test.
Native Windows/Linux execution, production deployment and authenticated
browser verification are recorded in `2026-09-profix-upgrade.md`, including
the unconfirmed transient empty-relay-response caveat.
