...
The authorization form uses a native HTML submit input plus a primary-pointer release fallback for Safari and embedded-browser flows that consume the first click while dismissing the software keyboard. The fallback runs normal form validation and is allowed by an exact Content Security Policy script hash. The form preserves ChatGPT's `resource` parameter through authorization and token exchange. An `authorization denied` response is generated only when the submitted recovery key does not match the current verifier. Because every Kill or rotation invalidates the previous key immediately, always use the newest key for the exact hostname being linked.
