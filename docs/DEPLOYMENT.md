...
The wrapper waits until local bootstrap, `executor status`, and `executor doctor --full` succeed. It always gives enrollment a separate protected helper-owned copy, then waits boundedly until `executor dashboard status --json` reports the actual relay as `connected` and local Agent/Broker state is healthy. The relay wait defaults to 30 attempts at one-second intervals; custom positive attempts and delays are accepted only when their conservative product is at most 300 seconds. Only then does it delete an explicitly designated original temporary file whose captured file identity still matches. Enrollment, status, relay timeout, local-health failure, or original-file replacement retains the current path and fails closed. Without the temporary flag the source file is always preserved. Enrollment failure does not invoke local rollback and does not remove a healthy existing installation. Loading the config through the CLI safely migrates v1/v2 state before enrollment.
