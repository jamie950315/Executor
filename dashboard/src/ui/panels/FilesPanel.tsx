...
export function FilesPanel({ call }: PanelProps) {
  ...
  const list = async (nextPath = path) => {
    ...
    try {
      const response = await call("filesystem_read", { action: "read_directory", path: nextPath, privilege: "owner" }, controller.signal);
      if (controller.signal.aborted) return;
      setEntries(parseEntries(response.result)); setPath(nextPath); setFilePath(null); setContent(""); setPreviewFailed(false); setStatus("Directory loaded");
    } catch { if (!controller.signal.aborted) setStatus("Directory unavailable"); }
  };
  useEffect(() => { void list("/"); return () => activeRead.current?.abort(); }, []); // one initial unrestricted root probe
...
function parseEntries(value: unknown): FileEntry[] {
  if (!Array.isArray(value)) throw new Error("Invalid directory listing");
  return value.map((item) => {
    const record = asRecord(item); const name = record?.Name ?? record?.name; const path = record?.Path ?? record?.path; const isDir = record?.IsDir ?? record?.isDir ?? record?.is_dir;
    if (typeof name !== "string" || typeof path !== "string" || typeof isDir !== "boolean") throw new Error("Invalid directory entry");
