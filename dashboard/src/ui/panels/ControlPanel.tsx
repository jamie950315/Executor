...
export function ControlPanel({ call, device, session, onSensitive, onKilled, onRotated, onRemoved }: PanelProps & {
  ...
  const load = async () => { try { const response = await call("control.status", {}, new AbortController().signal); const next = record(response.result); if (!next) throw new Error(); setSnapshot(next); setStatus("Lifecycle status verified"); } catch { setSnapshot(null); setStatus("Lifecycle status unavailable"); } };
  useEffect(() => { void load(); }, []);
  const closeConfirm = () => { setConfirm(null); setAcknowledged(false); setTypedName(""); };
  const run = async () => {
    if (!confirm || working) return; setWorking(true);
    try {
      if (confirm === "rotate" || confirm === "kill") {
        const value = await runSensitiveLifecycle(confirm === "rotate" ? "control.rotate" : "control.kill", device, session, call);
        onSensitive(value);
        if (confirm === "kill") { onKilled(); setStatus("Killed. Remote Resume is unavailable; use local rescue or executor CLI."); }
        else { onRotated(); setStatus("Credentials rotated. This browser must unlock the new generation again."); }
      } else {
        await removeDevice(device.device_id, new AbortController().signal); onRemoved(); setStatus("Device removed. Re-enroll from the host CLI with a new one-time enrollment token.");
      }
      closeConfirm();
    } catch { setStatus("Lifecycle action failed safely"); }
