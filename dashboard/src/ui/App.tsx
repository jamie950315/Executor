...
export function App() {
  ...
  const markState = useCallback((deviceID: string, uiState: DeviceView["uiState"], state?: DeviceRecord["state"]) => {
    setDevices((current) => current.map((device) => device.device_id === deviceID ? { ...device, uiState, ...(state ? { state } : {}) } : device));
  }, []);
  ...
  return <div className="app-frame"><a className="skip-link" href="#main-content">Skip to controls</a><header className="global-header"><div className="wordmark"><span>EX</span><strong>Executor</strong></div><div className="system-legend"><span className="live-dot" />Sovereign host control</div><div className="identity-block"><span>Access subject</span><strong>{session?.access_subject ?? "verifying"}</strong></div></header>
    <main id="main-content" className={selected ? "workspace-main" : "fleet-main"}>
      {selected && session ? <DeviceWorkspace device={selected} session={session} onBack={() => setSelectedID(null)} onLocked={() => { markState(selected.device_id, "locked"); setSelectedID(null); }} onKilled={() => markState(selected.device_id, "killed", "offline")} onRotated={() => { markState(selected.device_id, "locked"); setSelectedID(null); }} onRemoved={() => { setDevices((current) => current.filter((device) => device.device_id !== selected.device_id)); setSelectedID(null); }} onSensitive={setOneTimeResult} /> : <><section className="fleet-hero"><div><p className="eyebrow">Unified Dashboard / control plane</p><h1>Owner<br />Command</h1></div><div className="hero-copy"><p>Authenticated AI control across macOS, Windows, Linux, and WSL. Every device unlock belongs to one Access identity and one HttpOnly browser identity.</p><button onClick={() => void loadFleet()} disabled={loading}>{loading ? "Synchronizing…" : "Refresh fleet"}</button></div></section><p className="fleet-status" aria-live="polite">{status}</p><DeviceGrid devices={devices} onOpen={open} onUnlock={setUnlocking} /></>}
