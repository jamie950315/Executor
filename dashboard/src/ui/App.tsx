...
export function App() {
  ...
  const [manageHub,setManageHub]=useState(false);
  const hubUnlock=useRef(false);
  ...
  return <div className="app-frame"><a className="skip-link" href="#main-content">Skip to controls</a><header className="global-header"><div className="wordmark"><span>EX</span><strong>Executor</strong></div><div className="system-legend"><span className="live-dot" />Sovereign host control</div><div className="identity-block"><span>Access subject</span><strong>{session?.access_subject ?? "verifying"}</strong></div></header>
    <main id="main-content" className={selected ? "workspace-main" : "fleet-main"}>
      ...
      {selected && session ? <DeviceWorkspace device={selected} session={session} onFailure={reportFailure} onTabChange={(tab) => { workspaceTab.current = tab; }} onBack={() => setSelectedID(null)} onLocked={() => { markState(selected.device_id, "locked"); setStatus(`${selected.name} must be unlocked again in this browser.`); setSelectedID(null); }} onOffline={() => { markState(selected.device_id, "offline", "offline"); setStatus(`${selected.name} relay is offline.`); if (workspaceTab.current !== "files") setSelectedID(null); }} onKilled={() => { markState(selected.device_id, "killed", "offline", selected.generation + 1); setStatus(`${selected.name} was killed at generation ${selected.generation + 1}. Resume requires local rescue or executor CLI.`); setSelectedID(null); }} onRotated={() => { markState(selected.device_id, "locked", "online", selected.generation + 1); setStatus(`${selected.name} rotated to generation ${selected.generation + 1}. Unlock this browser again.`); setSelectedID(null); }} onRemoved={() => { setDevices((current) => current.filter((device) => device.device_id !== selected.device_id)); setStatus(`${selected.name} was removed. Re-enroll it locally from the host CLI with a new one-time token.`); setSelectedID(null); }} onSensitive={setOneTimeResult} /> : <><section className="fleet-hero"><div><p className="eyebrow">Unified Dashboard / control plane</p><h1>Owner<br />Command</h1></div><div className="hero-copy"><p>Authenticated AI control across macOS, Windows, Linux, and WSL. Every device unlock belongs to one Access identity and one HttpOnly browser identity.</p><button onClick={() => void loadFleet(true)} disabled={loading}>{loading ? "Synchronizing…" : "Refresh fleet"}</button></div></section><p className="fleet-status" aria-live="polite">{status}</p><div className="hub-toggle"><button disabled={!session} aria-expanded={manageHub} onClick={()=>setManageHub(value=>!value)}>{manageHub?"Close Hub manager":"Manage Pi5 Hub"}</button></div>{manageHub&&session&&<HubManager devices={devices} onUnlock={device=>{hubUnlock.current=true;setUnlocking(device);}} onNotice={setOperationNotice}/>}<DeviceGrid devices={devices} onOpen={open} onUnlock={device=>{hubUnlock.current=false;setUnlocking(device);}} /></>}
    ...
    {unlocking && session && <UnlockDialog device={unlocking} session={session} onDismiss={() => setUnlocking(null)} onUnlocked={() => { markState(unlocking.device_id, "unlocked"); setUnlocking(null); if(!hubUnlock.current){workspaceTab.current = "overview"; setSelectedID(unlocking.device_id);} hubUnlock.current=false; }} />}
